...
import getDataUri, { generateCertificate } from "../utils/dataUri.js";
...
export const login = catchAsyncError(async (req, res, next) => {
  ...
  if (!user) return next(new ErrorHandler("Incorrect Credentials, BTW it's easier than your ex's phone no.🤭", 401));

  const isMatch = await user.comparePassword(password);

  if (!isMatch)
    return next(new ErrorHandler("Incorrect Credentials, BTW it's easier than your ex's phone no.🤭", 401));
...
export const forgetPassword = catchAsyncError(async(req, res, next)=>{
    ...
    await sendEmail(user.email, "NexusTech Reset Password", message)
    res.status(200).json({
        success:true,
...
export const deleteMyProfile = catchAsyncError(async(req, res, next)=>{
  ...
  res.status(200)
  .cookie("token", null, {
    expires: new Date(Date.now()), httpOnly: true, sameSite:"none",
  }).json({
    success:true,
